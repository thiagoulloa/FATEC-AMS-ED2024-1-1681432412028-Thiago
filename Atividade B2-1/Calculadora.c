...
int main() {
    ...
    while (ptr != '0') {
        ...
        if (isdigit(ptr)) {
            ...
            printf(Caractere inválido na entrada '%c'n, ptr);
            exit(EXIT_FAILURE);
        }
    }

    if (pilha.top != 0) {
        printf(Erro Entrada inválida.n);
        exit(EXIT_FAILURE);
    }

    double resultadoNum;
    char resultadoExpr = pop(&pilha, &resultadoNum);
    printf(Resultado %s = %.2fn, resultadoExpr, resultadoNum);
